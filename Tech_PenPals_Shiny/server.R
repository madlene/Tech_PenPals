...
shinyServer(function(input, output, session) {
  ...
  calculatePayGapByQuantile <- function(quantile) {
    # Female Statistics
    ...
    statesMedianFemale2014 <- statesMedianFemale2014[ , grepl( quantile , names( statesMedianFemale2014 ) ) ]
    ...
    statesMedianMale2014 <- statesMedianMale2014[ , grepl( quantile , names( statesMedianMale2014 ) ) ]
    ...
    row.names(statesMedian2014) <- gsub(quantile, "", row.names(statesMedian2014))
    statesMedian2014$PayGap <- statesMedian2014$Male - statesMedian2014$Female
    
    return (statesMedian2014)
  }
  
  output$stateRank_PayGapByMedian <- renderText({
    statesMedian2014 <- calculatePayGapByQuantile(".50%")
...
#   Pencil Map d3.js  
#   observe({
#     if (input$ChooseStep9 == 0){
#       return()
#     } 
#     
#     cat(file=stderr(), "here \n")
#     
#     var_json = rjson::toJSON( calculatePayGapByQuantile(".50%") )
#     #statesMedian2014_json = rjson::toJSON( statesMedian2014 )
#     session$sendCustomMessage(type="jsondata", var_json)    
#   })
  

#   output$plot_USMap <- renderPlot({
#     df <- calculatePayGapByQuantile(".50%")
